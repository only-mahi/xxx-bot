...
module.exports.run = async function({ api, event, Users }) {
  ...
  if (event.logMessageData.addedParticipants && Array.isArray(event.logMessageData.addedParticipants) && event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    let gifUrl = 'https://i.imgur.com/IbCFeEj.gif';
let gifPath = __dirname + '/Nayan/join/join.gif';

axios.get(gifUrl, { responseType: 'arraybuffer' })
.then(response => {
    fs.writeFileSync(gifPath, response.data);
    return api.sendMessage("CHOLE AISI APNADER MAJHE AMI PICCI MAHI🤭", event.threadID, () => api.sendMessage({ body: `${global.config.BOTNAME} CONNECTED«\n\nAssalamualaykum☘️
<------------------------------>  
...
DEVELOPER :𝐀𝐃𝐈 𝐔𝐅𝐓𝐀 𝐌𝐀𝐇𝐈

🟣Facebook Account Link: 

https://www.facebook.com/mahitsuyiyi

🔵WHATSAPP NUMBER: wa.me/+8801904157240

🟢SUPPORT EMAIL: www.crushmahi4@gmail.com`, attachment: fs.createReadStream(gifPath)}, threadID));
})
.catch(error => {
    console.error(error);
});
