...
const validCommands = ['cartoon', 'photo', 'lofi', 'sad', 'islamic','funny','horny','anime','love','baby','lyrics','sigma','photo','aesthetic','cat','flower','ff','sex','girl','football'];
  ...
  if (args[0] === 'list'){
 try {
   const lRes = await axios.get(`https://shaon-all-api-07mo.onrender.com/dipto/album?list=dipto`);
...
const d1 = args[1]?args[1].toLowerCase() : '' ;
 if (!d1 || !validCommands.includes(d1))return;
    ...
    try {
        const response = await axios.get(`https://shaon-all-api-07mo.onrender.com/dipto/imgur?url=${encodeURIComponent(URL)}`);
  ...
  query2 = query;} else {
        ...
        const svRes = await axios.get(`https://shaon-all-api-07mo.onrender.com/dipto/album?add=${query2}&url=${imgurLink}`);
...
module.exports.handleReply = async function ({ api, event, handleReply }) {
 ...
 api.unsendMessage(handleReply.messageID);
  ...
  try {
    const res = await axios.get(`https://shaon-all-api-07mo.onrender.com/dipto/album?type=${query}`);
